= Prüfschritt 1.3.1a Strukturelemente für Überschriften
include::include/author.adoc[]
include::include/config.adoc[]

== Was wird geprüft?

Überschriften müssen als solche ausgezeichnet sein und die Inhalte der Seite erschließen. Falls verschiedene Überschriftsebenen ausgezeichnet werden, wird eine nachvollziehbare und sinnvolle Ebenenstruktur verwendet.

== Warum wird das geprüft?

Visuell können App-Inhalte durch Überschriften strukturiert werden. Dank dieser Strukturierung weiß der Benutzer, was zusammengehört, kann die Inhalte der Webseite leicht überblicken und gezielt auf Inhalte zugreifen, die ihn interessieren.
...
Der Prüfschritt ist anwendbar, wenn es auf der Seite Inhalte gibt, die durch Überschriften strukturiert werden können. Das ist bei informationsorientierten Apps in der Regel der Fall.
...
. Prüfen, ob Inhalte, die visuell als Überschriften zu erkennen sind auch als Überschriften ausgezeichnet sind. Die Hierarchie der ggf. eingesetzten Überschriftsebenen soll der Gliederung der Inhalte entsprechen. Die zu prüfenden Elemente werden angetippt dabei wird auf die Ausgabe des Screenreaders geachtet. Falls es sich um eine korrekt ausgezeichnete Überschrift handelt, gibt der Screenreader dies z. B. mit dem Wort „Überschrift“ aus. Falls zwischen Überschriftsebenen unterschieden wird, wird die entsprechende Ebene ebenfalls ausgegeben.
. Prüfen, ob die Überschriftsauszeichnung lediglich zur Erzeugung unterschiedlicher Schriftgrößen eingesetzt wird bzw. die Auszeichnung missbräuchlich eingesetzt wird.
...
Überschriften sollten korrekt geschachtelt sein (h1 gefolgt von h2 gefolgt von h3, und so weiter), um die Gliederung der Text-Inhalte abzubilden. Das Auslassen von Hierarchie-Ebenen muss aber kein Fehler sein, wenn dies durch die Gliederung der Inhalte gerechtfertigt ist. App-Entwickler haben evtl. derzeit keine Möglichkeit Überschriftsebenen mit dem Systemframework auszuzeichnen, bzw. konnte eine solche Gliederung in mobilen Apps noch nicht festgestellt werden.
...
Es werden Auszeichnungen für Überschriften eingesetzt, die Verschachtelung der Hierarchie-Ebenen ist aber irreführend, entspricht nicht der inhaltlichen Gliederung.

Nicht erfüllt:

Es gibt auf der App-Ansicht gegliederte Inhalte, die Überschriften werden jedoch nicht als solche ausgezeichnet.

In der App-Ansicht werden mehrere klar abgrenzbare Inhalte behandelt, es wurde aber nur pro forma eine Hauptüberschrift ausgezeichnet.
...
In diesem Prüfschritt geht es um den Einsatz von Überschriften zur Gliederung der App-Inhalte , damit diese leicht zu erfassen und mit Hilfsmitteln wie Screenreadern zu überspringen sind.

Im Prüfschritt 1 "Inhaltsbereiche strukuriert" geht es dagegen um die Strukturierung der Seitenbereiche durch Überschriften, WAI-Aria-Attribute oder Sprunglinks.
...
Der erste Bestandteil sagt, zu welchem Webangebot die Seite gehört. Er ist normalerweise für alle Dokumente eines Webangebots gleich, man braucht ihn, um die Seite von thematisch ähnlichen Seiten anderer Webangebote zu unterscheiden.
