...
Wenn die Software Videotelefonie unterstützt, soll nicht nur die Aktivtät von Sprechenden, sondern auch die von Gebärdensprach-Eingebenden angezeigt werden.
...
Der Prüfschritt ist anwendbar, wenn die App Videotelefonie unterstützt und eine Anzeige der Sprechaktivität von Sprechenden bietet.
...
. Prüfen, ob die App die Möglichkeit bietet, den Start eines Gebärden-Eingabe zu signalisieren oder die Eingabe von der App automatisch erkannt und in Folge der Aktivitäts-Zustand gesetzt werden kann.
. Prüfen, ob die Aktivität der Gebärden-Eingabe visuell wahrnehmbar ist.
