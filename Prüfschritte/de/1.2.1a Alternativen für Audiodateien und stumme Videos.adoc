...
[arabic]
. In der App die Audiodatei abspielen. Prüfen, ob Informationen vermittelt werden (z. B. über eine Kommentarstimme). Dann ist eine Transkription erforderlich.
...
[arabic]
. In der App die stumme Videodatei abspielen. Prüfen, ob Informationen vermittelt werden.
...
. Falls die Medienalternative in einer anderen Ansicht steht: gibt es zu ihr einen aussagekräftigen Link im unmittelbaren Kontext der Audio- oder Videodatei?
...
Nach der BITV 2 geht es in der Bedingung 1.1.1 "Nicht-Text-Inhalte" im Fall von zeitbasierten Medien um die Bereitstellung von kurzen beschreibenden Alternativtexten, die immer dann, wenn zeitbasierten Medien nicht angezeigt werden können, diese kurz identifizieren. Dies ist Gegenstand des Prüfschritts 1 "Alternativtexte für Grafiken und Objekte".
...
Im Prüfschritt 1 geht es schließlich um die Bereitstellung von zusätzlichen Audiodeskriptionen für synchronisierte Videos (also Videos mit Bild- und Tonspur). Dort geht es also um zusätzliche akustische Informationen über Bildinhalte, die nicht bereits über die synchrone Tonspur vermittelt werden.
