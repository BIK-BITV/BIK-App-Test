...
Wenn technische Dokumentation für die App bereitgestellt wird soll mindestens in eines der folgenden Formate barrierefrei
zur Verfügung gestellt werden, sofern sie nicht in die App selbst integriert ist:
...
Im App-Test-Verfahren kann jedoch lediglich technische Dokumentation geprüft werden, 
die in die App selbst integriert ist.

Dokumentation außerhalb der App muss dann in einem separaten Auftrag geprüft werden.
Wenn es sich um technische Dokumentation auf HTML-Basis handelt, kann das
das https://bitvtest.de/start.html[BIK-BITV-Test-Verfahren] genutzt werden.

=== Prüfung
Die Prüfung von in die App integrierter techniqscher Dokumentation 
folgt den übrigen Prüfschritten des Verfahrens, wie bei den anderen Ansichten der zu prüfenden Auswahl.
