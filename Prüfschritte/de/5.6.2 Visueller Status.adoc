...
Wenn es auf der speziellen Hardware, die zusammen mit der App genutzt wird, Umschalter oder dreiwertige Schalter gibt, die den Kontext der Ausgabe oder Eingabe beeinflussen, müssen diese Schalter auch visuell wahrnehmbar sein.
...
Virtualisierte Schalter innerhalb der App sind nicht von diesem Prüfschritt erfasst. Für die ist der Prüfschritt 11.4.1.2 Name, Rolle, Wert zuständig.

== Warum wird das geprüft?

Wenn der Zustand von Umschaltern bei der Nutzung der App mit spezieller Hardware relevant ist, soll dieser Zustand auch visuell erkennbar sein. Dies ist zum Beispiel dann der Fall, wenn ein Umschalter zwei visuell unterschiedbare Zustände hat. Beim Stummschalten des iPhones wird z.B. der Umschalter Richtung Geräterückseite geschoben, es erscheint zusätzlich ein schmaler roter Streifen im Ausschnitt, in dem sich der Schalter bewegt. Damit wird visuell signalisiert, dass der Ton abestellt ist.

== Wie wird das geprüft?

=== 1. Anwendbarkeit des Prüfschritts
Der Prüfschritt ist nur anwendbar, wenn für die Nutzung der Zustand von Hardware-Schaltern auf dem Ausgabegrät oder relevant spezieller zusätzulicher Hardware relevant sind. Dazu gehören etwa Schalter zum Feststellen der Bildschirmausrichtung oder zum Stummschalten.

=== 2. Prüfung
. Gibt es aud dem Ausgabegerät oder zusätzlich genutzter hardware Umschalter, die generell den Kontext der Ausgabe oder Eingabe beeinflussen (z.B. die Bildschirm-Ausrichtung feststellen oder den Ton stumm schalten)?
. Falls das zutrifft: Mittels Sichtprüfung für solche Umschalter prüfen, ob der Zustand visuell ermittelbar ist.

=== 3. Bewertung
==== Nicht erfüllt
* Für die Nutzung der App ist ein physischer Umschalter auf dem Ausgabegerät oder auf zusätzlicher Hardware relevant, der den Kontext der Ausgabe oder Eingabe beeinflusst und dessen Zustand ist nicht visuell ermittelbar ist.
