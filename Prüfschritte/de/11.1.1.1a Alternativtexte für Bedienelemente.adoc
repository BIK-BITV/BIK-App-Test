...
Entwickler müssen Grafiken mit passenden Text ergänzen, damit Hilfsmittelsoftware den Text anstelle der Grafik z. B. in Braille und Sprache ausgeben kann. Das UI-Element wird dabei um ein Accessibility-Label ergänzt. Bei komplexen Informations-Grafiken soll ein Alternativtext oder eine andere Alternative (z.B. die tabellarische Ansicht eines Diagramms) für nicht-sehende Nutzende einen möglichst äquivalenten Informationsgehalt vermitteln.
...
Handelt es sich um eine informationstragende Grafik, sollte diese im Alternativtext bzw. im unmittelbaren Kontext beschrieben sein. Bei interaktiven Grafiken soll (ggf. zusätzlich) das Ziel bzw. die hinterlegte Funktion angeben sein.

. App mit zu prüfender Ansicht öffnen.
. Screenreader starten und Fokus auf jedes grafische Bedienelement setzen.
. Prüfen, ob für die Bedienelemente Alternativtexte ausgegeben werden und diese (auch) das Ziel bzw. die Funktion des Bedienelements beschreiben.
...
. Bei Informationstragenden Grafiken wird ein äquivalenter Alternativtext ausgegeben oder ist im unmittelbaren Kontext der Grafik verfügbar.

Gleichwertige (äquivalente) Alternativtexte:

Entscheidend ist: die App soll benutzbar sein, wenn Grafiken, oder Bilder oder Objekte durch die entsprechenden Alternativtexte oder Textalternativen ersetzt
sind.

In der Regel bedeutet das:

* Bei Schriftgrafiken soll der Alternativtext den Text der Schriftgrafik wiederholen.
* Bei Symbolen soll der Alternativtext das Symbol nicht beschreiben, sondern ersetzen. Also zum Beispiel der Alternativtext "Suchen" für ein Lupen-Symbol, das einen Dialog mit Sucheingabefeld öffnet.
* Bei Objekten, die nicht angezeigt werden können, sollen der Alternativtext eine kurze Beschreibung oder Identifizierung bieten. Sinnvoll ist es, dass eine
...
* Beschreibung des abgebildeten Gegenstandes in den Alternativtext (wenn der abgebildete Gegenstand wichtig ist und daraus das Linkziel hervorgeht, zum
  Beispiel Logo)
* Ziel des Links in den Alternativtext
* Beschreibung des abgebildeten Gegenstandes und Ziel des Links im Alternativtext (wenn beides wichtig ist)
...
* Wenn es sich nicht um ein zusammengesetztes Bedienelement handelt, der Screenreader im Lesemodus also beide Elemente  nacheinander fokussiert und ausgibt, soll der Alternativtext der Grafik nicht einfach die nebenstehende Beschriftung (Linktext) wiederholen. Je nach Inhalt der Abbildung kann der Alternativtext dann 
