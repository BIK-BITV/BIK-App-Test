...
Grafische Bedienelemente müssen Textalternativen haben. Textalternativen für verlinkte Grafiken oder Teaserbilder sollen das Ziel des Links bezeichnen. Textalternativen für grafische Schaltflächen (Buttons) sollen die Aktion bezeichnen, die der Button auslöst. Ist eine Grafik in ein Bedienelement eingeschlossen, dessen Linktext bzw. dessen hinterlegte Beschriftung das Linkziel bzw. die Funktion bereits ausreichend beschreibt, sollte das Bild aus der programmatischen Ausgabe durch geeignete Mittel entfernt werden.

== Warum wird das geprüft?

Für Screenreader-Nutzende sind Grafiken selbst nicht zugänglich. Die Textalternative tritt deshalb an die Stelle der Grafik, sie soll die Grafik ersetzen und bei Bedienelementen das Linkziel bzw. die Aktion bezeichnen.

Entwicklerinnen und Entwickler müssen Grafiken mit passenden Text ergänzen, damit Hilfsmittelsoftware den Text anstelle der Grafik, z. B. auf der Braille-Zeile oder als Sprache ausgeben kann. Das UI-Element wird dabei um ein Accessibility-Label ergänzt.

== Wie wird geprüft?

=== 1. Anwendbarkeit des Prüfschritts

Der Prüfschritt ist anwendbar, wenn Grafiken als Bedienelemente (z.B., Icons in Menüs oder Werkzeugleisten, Logos, Teaserbilder) eingesetzt werden oder in bzw. auf Bedienelementen eingebunden werden.
...
Ist ein Bedienelement eine Grafik ohne visuell sichtbaren Text (alleinstehendes Icon oder Symbol), so sollte eine Textalternative vorhanden sein, die das Ziel bzw. die Aktion, die ausgeführt wird, beschreibt. In der Praxis ist dies Text, der über ein Accessibility-Label bereitgestellt wird.
...
. Prüfen, ob für die Bedienelemente Textalternativen ausgegeben werden und diese das Ziel bzw. die Funktion des Bedienelements beschreiben.
...
* Ausführliche Beschreibungen von Abbildungen sollen nicht in die Textalternative, sondern im Kontext der Abbildung zur Verfügung gestellt werden (etwa als Text direkt darunter, in einem Ausklappbereich, oder auch in einem Link zu einer Ansicht, die die Alternative enthält).
...
- iOS 14+: Einstellungen > Bedienungshilfen > VoiceOver > VoiceOver-Erkennung. (Diese Funktion wird nur auf iPhone XS/XR-Modellen oder später unterstützt).
