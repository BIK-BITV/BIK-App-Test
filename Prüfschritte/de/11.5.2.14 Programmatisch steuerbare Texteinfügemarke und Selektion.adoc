...
Bei Interface-Elementen der App, die Texteingaben und Textbearbeitung vorsehen, muss die Position des Fokus bzw. der Texteinfügemarke und sowie die Textauswahl auch für Hilfsmitteltechnologien änderbar sein. Geprüft wird dies mithilfe der integrierten Screenreader VoiceOver (iOS) und TalkBack (Android) und der Aktivierung der jeweiligen Gesten-Befehle für Fokusversetzung und Textauswahl. Ob die geänderte Position oder Texauswahl programmatisch ausgegeben wird, ist Gegenstand des Prüfschritts 11.5.2.13. Beide Prüfschritte gehören eng zusammen, die Prüfung ist im wesentlichen identisch.
