= Prüfschritt 1.3.3.8 Zugängliche Authentifizierung (Minimum)
...
Nutzende sollen sich mit einer zugänglichen, einfachen und sicheren Methode auf einem zugangsgeschützten Webangebot anmelden können. 
...
oder die Anmeldung mithilfe der autocomplete-Funktion (des Browsers) durchgeführt werden kann. 
...
Zum Beispiel erfordert die Authentifizierung in einem Webviewmeinen Verifizierungscode, der als SMS an ein Mobiltelefon gesendet wird. 
