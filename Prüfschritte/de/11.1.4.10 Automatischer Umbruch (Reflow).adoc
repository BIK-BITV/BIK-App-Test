...
Die Viewport-Breite ändert sich auch, wenn die Ausrichtung des Geräts geändert wird und sich Inhalte an die neue Ausrichtung anpassen. 
...
Im Kontext der https://www.w3.org/groups/tf/wcag2ict[WCAG2ACT Taskforce] der Accessibility Guidelines Working Group läuft eine https://github.com/w3c/wcag2ict/issues/98[Diskussion, ob und wie Reflow auch auf Nicht-Web-Inhalte anzuwenden wäre].
...
* Werden Apps getestet, die Layouts für breitere Ansichten bieten *und* eine Zoomfunktion implementieren, ist zu prüfen, ob die Querformat-Ansicht auf dem Smartphone bereits ein abweichendes Layout bietet.
  Z.T. werden Apps nur in der Hochkant-Ansicht angeboten (und erfüllen damit nicht Anforderung 11.1.3.4 "Keine Beschränkung der Bildschirmausrichtung") oder sie zeigen die App-Inhalte auch im Querformat nur in einem Teil des Displays, der der Viewportbreite bei Hochkant-Ansicht enspricht.
