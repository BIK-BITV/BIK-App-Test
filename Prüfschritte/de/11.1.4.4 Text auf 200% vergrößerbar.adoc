...
* Mit der Funktion "größerer dynamischer Text" des Betriebssystems kann der Text vergrößert werden.
...
* Die Zoomfunktion (Vergrößerung des gesamten Inhalt des Bildschirms) des Betriebssystems wird in der Regel eine 200%ige Vergrößerung ermöglichen. Funktioniert die Zoomfunktion mit der zu testenden Software, ist dieser Prüfschritt bestanden. 
Native mobile Plattformen verfügen über eine Funktion "größerer dynamischer Text". Apps, die diese Funktion unterstützen, passen sich an die festgelegte Lesegröße an. Diese Funktion wird häufig verwendet, da die Bedürfnisse von sehbehinderten Nutzenden durch die Zoom-Funktion der Plattform nicht vollständig abgedeckt werden. Wir empfehlen daher dringend, dass native Apps diese Funktion unterstützen. Probleme die bei der Nutzung der dynamischen Textfunktion entstehen, sollten zumindest angemerkt werden.

* Die Prüfung der Schriftvergrößerung durch ein eigenes Bedienelement in der zu testenden App-Ansicht erfolgt nur unter der Bedingung, dass das Bedienelement alle Anforderungen an Barrierefreiheit erfüllt (zum Beispiel tastaturbedienbar, erfüllt die Kontrastanforderungne etc.)
...
=== Artikel: 
* https://medium.com/@r.whitaker/swiftui-accessibility-dynamic-type-d9b99ae87334[SwiftUI Accessibility: Dynamic Type, Rob Whiteker, 2019]
