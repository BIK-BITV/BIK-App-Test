...
* Sehbehinderte Nutzende brauchen ggf. mehr Zeit, Inhalte zu lesen. Eingeblendete Inhalte sollten deshalb solange zur Verfügung stehen, bis sie vom Nutzer geschlossen werden (etwa über Weiter-Tabben, Wegbewegen des Zeigers von auslösenden Element und eingeblendetem Inhalt, oder explizites Schließen über die Tastatur).

== Wie wird geprüft?

=== 1. Anwendbarkeit des Prüfschritts

Der Prüfschritt ist derzeit nicht anwendbar, da bisher keine derartigen Einblendungen in nativen Apps für iOS, iPadOS und Android möglich sind.

Dies liegt daran, dass die primäre Eingabemethode auf den mobilen Geräten auf Berührungsgesten basiert und dabei keine Zustände existieren, die entsprechende Einblendungen ermöglichen würden. Sobald Maus- und Tastaturnutzung an Touch-Devices zunehmen und Einblendungen im Sinne dieses Prüfschritts Verbreitung finden, ist dieser Prüfschritt anwendbar.

Dann gilt:

Der Prüfschritt ist anwendbar, wenn die App zusätzliche Inhalte enthält, die bei Fokussierung mittels Tastatur oder Zeiger
eingeblendet werden, etwa Custom-Tooltips oder Ausklapp-Menüs, die bereits bei Fokussierung ausklappen.
...
Wenn Inhalte durch Hover eingeblendet werden, sollten sie in der Regel auch bei Tastaturfokus eingeblendet werden, falls sie nicht alternativ zur Verfügung
gestellt werden.
