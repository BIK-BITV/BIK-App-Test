...
Wenn in einer Ansicht mehrere Layout- oder Schmuckgrafiken auftauchen, kann dies schnell zu
...
. Prüfen ob Layoutgrafiken oder dokorative Grafiken eingesetzt werden
. Screenreader starten und Fokus auf die ensprechende Grafik setzen
. Prüfen, ob beim Antippen der Layoutgrafiken eine Ausgabe durch den Screenreader
  stattfindet.

Der Screenreader sollte bei Layoutgrafiken oder dokorativen Grafiken nichts ausgeben.
...
Die Information, dass da irgendwo Abstände zwischen Elementen in der Ansicht sind,
ist nutzlos.
