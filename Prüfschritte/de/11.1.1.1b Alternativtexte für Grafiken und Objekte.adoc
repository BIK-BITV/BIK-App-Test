...
soll der Alternativtext zumindest eine beschreibende Identifizierung des
Inhalts ermöglichen.
...
Wenn Objekte (etwa Video- oder Audio-Dateien) nicht angezeigt
...
Bezug zum Thema der App-Ansicht.
Ihr Informationsgehalt und ihr Nutzen für das Verständnis ist aber
nicht klar.
...
. Prüfen, ob dieser bei den entsprechenden Elementen auf dem Bildschirm aussagekräftige Alternativtexte
  ausgibt.
...
(vorangehende, folgende) Text und eine im unmittelbaren Kontext des Bildes angebotene verlinkte und klar
benannte Textalternative.
