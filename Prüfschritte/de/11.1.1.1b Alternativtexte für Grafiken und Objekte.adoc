...
Informative Grafiken und Bilder müssen Textalternativen haben. 
Die Textalternativen ersetzen das Bild, sie sollen (soweit wie möglich) die gleiche Information vermitteln wie das Bild.
...
Die Textalternative tritt dann an die Stelle der Grafik, sie soll die Grafik inhaltlich ersetzen können.

Wenn Objekte (etwa Video- oder Audio-Dateien) nicht angezeigt werden können, 
weil sie z.B. nicht geladen werden können, sollen kurze beschreibende Textalternativen dem Nutzer eine Identifikation der Inhalte ermöglichen.
...
sind diese fast nie als bloße Schmuckgrafik anzusehen und brauchen deshalb eine Textalternative. 
Dies ermöglicht blinden und sehenden Nutzern, sich gemeinsam auf diese Elemente zu beziehen. 
Die Textalternative muss aber nicht expliziter sein als das, was für den sehenden Nutzer erkenntlich ist.
...
* Bei Symbolen oder Zeichen (Logos) soll die Textalternative sagen, dass ein Symbol, 
...
* Bei Diagrammen oder technischen Zeichnungen sind unter Umständen ausführlichere Erläuterungen erforderlich.
  Textalternativen sind dafür nicht geeignet, sie sollen in der Regel 80 Zeichen nicht überschreiten. 
  In der Textalternative steht dann nur, was dargestellt ist und wo (etwa: "Details im anschließenden Text"). 
...
Generell gilt: Textalternativen sollen kurz sein. 
Ausführliche Beschreibungen von Abbildungen sollen nicht in die Textalternative, 
sondern im Kontext der Abbildung zur Verfügung gestellt werden.

==== Angemessenheit von Textalternativen im Kontext

Wenn die Textalternative kein angemessener Ersatz für die Grafik oder das Bild sein kann, 
...
der dazugehörige (vorangehende, folgende) Text und eine im unmittelbaren Kontext des Bildes angebotene verlinkte und klar benannte Textalternative. 
...
Eine kurze Textalternative mit Bezeichnung des Bildes ist auch bei Nutzung von Textalternativen im Kontext erforderlich. 
Die Textalternative sagt, dass an dieser Stelle etwas Bestimmtes abgebildet ist. 
Sie macht klar, worauf sich die Erläuterung im Kontext bezieht.

==== Beurteilung der Angemessenheit ====

Um die Angemessenheit der Textalternative einschätzen zu können, 
