...
nachvollziehbar sein. Sie soll der visuellen Reihenfolge entsprechen.

== Warum wird das geprüft?

Apps sollen für alle Nutzenden zugänglich sein. Das heißt, das sie verschiedene Arten der Eingabe unterstützen müssen: nicht nur Touch, sondern auch Tastaturbedienung, Switch-Steuerung oder Sprachsteuerung. Probleme gibt es meistens mit der Tastaturbedienung, denn die Mehrzahl der Nutzer arbeitet mit Touch-Eingaben, daher wird oft nur an diese gedacht. Auf die Tastaturbedienbarkeit angewiesen sind zum Beispiel viele motorisch eingeschränkte Menschen oder blinde Nutzer. Für assistive Technologien ist die Tastaturnutzbarkeit häufig eine Voraussetzung. 

Bei Apps soll die Fokus-Reihenfolge im Wesentlichen die visuelle Reihenfolge der Bedienelemente abbilden. Durch eine nicht nachvollziehbare Reihenfolge der Bedienelemente kann die Tastaturbedienbarkeit erheblich beeinträchtigt werden.

Wenn in der Ansicht dynamische Elemente wie etwa modale Dialoge, Popup- bzw. Schubladen-Menüs oder Fehlermeldungen vorkommen, muss der Tastaturfokus in diese ersetzt werden, damit Tastaturnutzende mit diesen interagieren können. Beim Schließen dynamischer eingefügter Elemente muss der Fokus auf das auslösende Element zurückversetzt werden.
...
damit Screenreader auf die hinzugefügten Elemente stoßen und diese ausgeben. Werden dynamische Inhalte zusätzlich auf der Ansicht eingefügt, so dass sie andere Inhalte adecken, muss die App dafür sorgen, dass der Fokus dorthin versetzt wird und so auch blinde Nutzer die Änderung mitbekommen.
...
. Interaktive Elemente sollten nach Fokussierung aktiviert werden, um auch jene Elemente zu erfassen, die Unteransichten (etwa Ausklappbereiche, Menüs, Dialoge usw.) aufrufen, denn die Fokusreihenfolge muss auch in diesen geprüft werden. Hierbei kommen fallweise Tabulator oder Pfeiltasten, auch im Wechsel, zum Einsatz.
