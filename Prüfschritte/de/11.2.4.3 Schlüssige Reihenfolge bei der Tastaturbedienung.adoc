...
. Falls dynamische Inhalte (Dialoge, Popovers, Menüs) eingefügt werden: Wird der Fokus an den Beginn des zusätzlichen Inhalts versetzt?
...
iOS: Für Reiter-Navigationen (tab panels) im Fußbereich bleibt der Fokus häufig nach Aktivierung eines Tabs auf diesem Tab und wird nicht an den Beginn des eingeblendeten Panels versetzt. Dies ist ein in vielen Apps etabliertes iOS-Pattern und sollte deshalb nicht negativ bewertet werden.

==== 3.1 Hinweise zur Prüfung von Webviews
...
Die Prüfung der Fokusreihenfolge sollten blinde Prüfende grundsätzlich zusammen mit der sehenden Assistenz vorehmen. Beide durchlaufen gemeinsam und sorgfältig sämtliche Bedienelemente der Ansicht, wobei die sehende Assistenz darauf achtet, dass keine visuell verfügbaren Bedienelemente bei der Tastaturnutzung übersprungen werden oder nicht erreicht werden können. Fehlen aud Fokuspunkten zugängliche Namen oder Rollen, können diese Mängel in den entsprechenden Prüfschritten festgehalten werden. 
Bedienelemente sollten aktiviert werden, umd jene zu ermitteln, ob sie Unteransichten (etwa Ausklappbereiche, Menüs, Dialoge usw.) aufrufen, denn die Fokusreihenfolge muss auch in diesen geprüft werden. Hiezu kommen whalweise Tabularor oder Pfeiltasten zum Einsatz.
