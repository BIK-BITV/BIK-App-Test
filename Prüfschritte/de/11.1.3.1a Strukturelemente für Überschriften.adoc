...
Falls verschiedene Überschriftsebenen existieren, besteht grundsätzlich die
Anforderung, dass die Strukturierung sinnvoll und nachvollziehbar ist.

Aktuell können verschiedene Überschriftsebenen lediglich in Web-Views von Apps
unter iOS / iPadOS und Android realisiert werden.
In Nativen Ansichten können derzeit Überschriften nicht programmatisch in
Ebenen unterteilt werden.
Aufgrund technischer Begrenzungen können Entwickler hier nur ein UI-Element als
einfache Überschrift auszeichnen.
...
Inhalte der Ansicht leicht überblicken und gezielt auf Inhalte zugreifen,
...
Weitere Anpassungen sind denkbar, z. B. könnten Screenreader zukünftig
Überschriften mit einer anderen Stimme vorlesen, um diese als solche
hervorzuheben.
...
. Prüfen, ob Inhalte, die visuell als Überschriften zu erkennen sind auch als
  ...
  Die Navigationseinstellung des Screenreaders wird auf "Überschriften"
  gesetzt.
  Anschließend navigiert man durch alle Überschriften der Ansicht und prüft
  die Ausgabe des Screenreaders.
  Falls es sich um eine korrekt ausgezeichnete Überschrift handelt, gibt der
  Screenreader dies mit dem Wort "Überschrift" o. ä. aus.
  Falls zwischen Überschriftsebenen unterschieden wird, wird die entsprechende
  Ebene ebenfalls ausgegeben.
  Es handelt sich dann um ein Web-View.
  Die Hierarchie der eingesetzten Überschriftsebenen soll dann der Gliederung
...
Wird vom Screenreader zusätzlich zur Rolle "Überschrift" auch die Überschriftenebene ausgegeben, handelt es sich um ein Web-View, die auf HTML
basiert. Überschriften sollten dann korrekt geschachtelt sein (h1 gefolgt von h2 gefolgt von h3, und so weiter), um die Gliederung der Inhalte abzubilden.
...
* Es werden Auszeichnungen für Überschriften eingesetzt, die Verschachtelung
  ...
  inhaltlichen Gliederung (Nur bei Web-Views, siehe <<Hinweise>>).
