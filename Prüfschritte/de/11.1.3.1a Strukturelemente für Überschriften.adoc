...
Die eingesetzten Überschriften sollten die strukturelle Hierarchie des Inhalts richtig wiedergeben. Aktuell können jedoch unterschiedliche Überschriftsebenen unter iOS / iPadOS und Android lediglich in Web-Views mit Web-Techniken (`h1` - `h6` bzw. durch den Einsatz von ARIA-Rollen und -Attributen) realisiert werden.
...
. Die Navigationseinstellung des Screenreaders auf "Überschriften" setzen. Anschließend durch all Überschriften der Ansicht navigieren und die Ausgabe des Screenreaders prüfen. 
...
* Es werden Auszeichnungen für Überschriften eingesetzt. Bei Web-Views (siehe <<Hinweise>>) ist die Verschachtelung der Hierarchie-Ebenen jedoch irreführend und entspricht nicht der inhaltlichen Gliederung.
...
http://www.w3.org/TR/WCAG21-TECHS/H42.html[WCAG 2.0 Technik H42: Using
