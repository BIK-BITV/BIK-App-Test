...
Überschriften müssen als solche ausgezeichnet sein und die Inhalte der App-Ansicht erschließen. Falls verschiedene Überschriftsebenen existieren, besteht grundsätzlich die Anforderung, dass die Strukturierung sinnvoll und nachvollziehbar ist.

Aktuell können verschiedene Überschriftsebenen lediglich in Web-Views von Apps unter iOS / iPadOS und Android realisiert werden.
In nativen Ansichten können derzeit Überschriften nicht programmatisch in Ebenen unterteilt werden.
Aufgrund technischer Begrenzungen können Entwickler hier nur ein UI-Element als einfache Überschrift auszeichnen.

== Warum wird das geprüft?

Visuell können App-Inhalte durch Überschriften strukturiert werden. Dank dieser Strukturierung weiß der Benutzer, was zusammengehört, kann die Inhalte der Ansicht leicht überblicken und gezielt auf Inhalte zugreifen, die ihn interessieren.

Benutzer, die diese visuelle Ordnung nicht nutzen können, zum Beispiel, weil sie blind sind oder nur einen kleinen Ausschnitt der Seite sehen können, sind darauf angewiesen, dass die Struktur unabhängig von der Darstellung auf dem Bildschirm zugänglich und nutzbar ist. Die Verwendung von Überschriften-Elementen ist dafür eine wichtige Voraussetzung.
...
Weitere Anpassungen sind denkbar, z.B. könnten Screenreader zukünftig Überschriften mit einer anderen Stimme vorlesen, um diese als solche hervorzuheben.
...
. Prüfen, ob Inhalte, die visuell als Überschriften zu erkennen sind, auch als Überschriften ausgezeichnet sind. Die Navigationseinstellung des Screenreaders wird auf "Überschriften" gesetzt. Anschließend navigiert man durch all Überschriften der Ansicht und prüft die Ausgabe des Screenreaders. Falls es sich um eine korrekt ausgezeichnete Überschrift handelt, gibt der Screenreader dies mit dem Wort "Überschrift" o.ä. aus. 
. Auf Webviews kann zwischen Überschriftsebenen unterschieden werden, hier wird die entsprechende Überschriftenebene ebenfalls ausgegeben. Die Hierarchie der Überschriften soll dann der Gliederung der Inhalte entsprechen. Der Aufruf des Rotors (iOS) bzw. von Kontextmenü / Navigation (Android) gibt einen Hinweis, ob es sich um ein Webview oder eine native Ansicht handelt. Bei Webviews werden in der Regel mehr Navigationsmöglichkeiten angezeigt.
. Prüfen, ob die Überschriftsauszeichnung lediglich zur Erzeugung unterschiedlicher Schriftgrößen eingesetzt wird und damit die Auszeichnung missbräuchlich eingesetzt wird.
...
* Es werden Auszeichnungen für Überschriften eingesetzt, die Verschachtelung der Hierarchie-Ebenen ist aber irreführend, entspricht nicht der inhaltlichen Gliederung (Nur bei Web-Views, siehe <<Hinweise>>).

==== Nicht erfüllt:

* Eine vorhandene visuelle Hauptüberschrift der Ansicht ist nicht als als Überschrift ausgezeichnet.
