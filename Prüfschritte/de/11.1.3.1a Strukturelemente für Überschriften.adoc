...
. Prüfen, ob Inhalte, die visuell als Überschriften zu erkennen sind, auch als Überschriften ausgezeichnet sind. Die Navigationseinstellung des Screenreaders wird auf "Überschriften" gesetzt. Anschließend navigiert man durch all Überschriften der Ansicht und prüft die Ausgabe des Screenreaders. Falls es sich um eine korrekt ausgezeichnete Überschrift handelt, gibt der Screenreader dies mit dem Wort "Überschrift" o.ä. aus. Falls zwischen Überschriftsebenen unterschieden wird (etwa bei Webviews), wird die entsprechende Ebene ebenfalls ausgegeben. Die Hierarchie der eingesetzten Überschriftsebenen soll dann der Gliederung der Inhalte entsprechen.
...
Wird vom Screenreader zusätzlich zur Rolle "Überschrift" auch die Überschriftenebene ausgegeben, handelt es sich um ein Web-View, die auf HTML basiert. Überschriften sollten dann korrekt geschachtelt sein (h1 gefolgt von h2 gefolgt von h3, und so weiter), um die Gliederung der Inhalte abzubilden. Das Auslassen von Hierarchie-Ebenen muss aber kein Fehler sein, wenn dies durch die Gliederung der Inhalte gerechtfertigt ist.

=== 4. Bewertung

==== Erfüllt:

* Visuelle Überschriften auf der Seite sind asuch semantisch als Überschriften umgesetzt.
