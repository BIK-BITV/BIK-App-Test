...
Wenn Apps Formularelemente verwenden, tragen diese in der Regel eine visuelle Beschriftung, die Nutzwenden sagt, was hier eingetragen werden soll. Für Hilfsmittelnutzende muss diese Beschriftung auch nicht-visuell, also programmatisch, ermittelbar sein. Eingabefeld und Beschriftung sollten bei Wischgesten-Bedienung mit aktiviertem Screenreader im besten Fall nur ein Fokuspunkt sein. Der Screenreader liest dann bei Fokussierung des Eingabefeldes die hinterlegte oder verknüpfte Feld-Beschriftung vor.
...
. Prüfen, ob bei Fokussierung von Feldern oder anderen Formularelementen die Feld-Beschriftungen und (wenn vorhanden) Gruppenbeschriftungen korrekt ausgegeben werden.
