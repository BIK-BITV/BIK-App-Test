...
== Was wird geprüft?
Wenn es in der App nicht-visuell wahrnehmbare Umschalter oder dreiwertige Schalter gibt, die den Kontext der Ausgabe oder Eingabe beeinflussen, müssen diese Schalter auch visuell wahrnehmbar sein.
...
. Falls das zutrifft: Mittels Sichtprüfung für jedes dieser Bedienelemente prüfen, ob es sichtbar ist und sein Status visuell ermittelbar ist.
...
Der Status der Shift-Taste bei virtuellen Tastaturen (festgestellt) wird bei allen uns bekannten Tastaturen visuell vermittelt. Da App-Entwickler keinen Einfluss auf installierte Tastaturen haben, werden sie in diesem Prüfschritt nicht berücksichtigt.
