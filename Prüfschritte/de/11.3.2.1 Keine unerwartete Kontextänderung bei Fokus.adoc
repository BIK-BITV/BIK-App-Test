...
Kontextänderungen in der App können Nutzer ablenken und verwirren oder auch
...
Orientierung des Benutzers ebenfalls beeinträchtigen.
...
. Mit der Tastatur (ggf. externe Bluetooth-Tastatur) mittels Tab-Taste alle
  Elemente in der Ansicht durchgehen und dabei prüfen, ob es eine unerwartete
  Kontextänderung gibt (etwa Pop-up-Fenster oder automatisches Abschicken von
  Formularen)

=== Hinweise

Der Prüfschritt bezieht sich nur auf das Öffnen von neuen Ansichten , nicht
auf Elemente, die den Inhalt überlagern (Stichwort Lightbox).
