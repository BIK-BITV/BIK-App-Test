...
* Eine Ausnahme bilden Tab-Leisten unten im Display, bei denen die Aktivierung eines Tabs ein neues Panel öffnet, wobei der Fokus auf dem aktivierten Tab verbleibt und nicht an den Beginn des Panels versetzt wird. Dieses Verhalten ist in vielen Apps (bei iOS etwa in Apples Standard-Apps wie App Store, Music oder Health, bei Androind im Play Store, der YouTube und der Fotos App) zu finden und kann deshalb als von Nutzenden erwartbar vorausgesetzt werden. Zur Fokussierung des Panels muss hier der Display im oberen Bereich berührt werden.
