...
. Prüfen, ob alle Spalten- und Zeilenüberschriften korrekt ausgezeichnet sind. Dazu wird die Zeilen- bzw. Spaltenüberschrift angetippt, dabaei wird darauf geachtet, was der Screenreader ausgibt. Wenn der Screenreader „Zeilenüberschrift“, „Spaltenüberschrift“ oder ähnliches als Zusatz spricht, sind die entsprechenden Überschriften korrekt ausgezeichnet.

Die Mindestanforderung: Offenkundige, unverzichtbare und visuell hervorgehobene Überschriften müssen ausgezeichnet sein (siehe Hinweis Woran erkennt man Zeilenüberschriften?).
...
Muss beim Zugriff auf Daten auf den Inhalt der Zeile oder Spalte Bezug genommen werden, ist es ohne Berücksichtigung der Randspalte nicht möglich, wenigstens einen Teil der unterschiedlichen Werte einer anderen Spalte zu interpretieren? Dann enthält die Randspalte Überschriften, sie muss entsprechend ausgezeichnet werden. (Beispiel: im Wertebereich der Tabelle finden sich ausschließlich Preisangaben.)

Die Randspalte kann abweichend von der visuellen Darstellung als Überschrift ausgezeichnet werden, wenn es Sinn macht, beim Vergleich von Werten anderer Spalten auf sie Bezug zu nehmen. (Beispiel: Tabelle mit Produkten, Anbieter und Modellbezeichnung sind als Überschriften ähnlich brauchbar.)
...
Für tabellarische Daten wird keine Tabellensemantik verwendet. Tabellen werden z. B. mittels Tabstops umgesetzt.
