...
* Bei VoiceOver ist dies VoiceOver-Erkennung und darunter die Optionen Bildbeschreibung und Bildschirmerkennung. Beide sollten deaktiviert sein. 
* Bei Talkback heipt der Punkt "Automatische beschreibungen" mit den Optionen "Symbole beschreiben" unf "Text in Bildern beschreiben". Beide sollten deaktiviert sein.
. Den Screenreader aktivieren und mit Hilfe der Wischgeste den Fokus der Reihe nach auf alle Bedienelemente mit Textbeschrfitung setzen.
