...
. Prüfen, ob alle wesentlichen Funktionen über die Tastatur erreicht und aktiviert werden können. In der Regel heißt das, das Elemente fokussiert und dann über ENTER oder Leertaste aktivert werden können. Wo Elemene nicht erreichbar sind, stehen äquivalente Wege zum Auslösen der Funktion über die Tastatur zur Verfügung.
